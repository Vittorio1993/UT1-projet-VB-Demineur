Public Class FormDemineur
    Private Taille As Byte = 9
    Private NbMine As Byte = 10
    Dim temp As Integer
    ...
    Private Sub FormDemineur_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'le handles de clickJeu est construit dynamiquement par l'instrcution AddHandler
    ...
    Private Sub Affiche(ByVal i As Byte, ByVal j As Byte)
        'cette procédure permet d'afficher une case
        For Each element In pnlJeu.Controls
    ...
    Private Sub ButtonEnabled(ByVal i As Byte, ByVal j As Byte)
        'cette procédure permet de voir le enabled d'un button
        For Each element In pnlJeu.Controls
            If element.tag = (j - 1) * Taille + i Then
    ...
    Private Sub affichevoisin(ByVal i As Byte, ByVal j As Byte)
        'Cette procédure permet de decouvrir le nombre de mine autour de la case 
        Dim a As Byte
        Dim b As Byte
        For a = i - 1 To i + 1
            For b = j - 1 To j + 1
                ...
                Else
                    ...
                    If Jouer(lig, col) = -1 Then
                        ...
                        If repondre = 6 Then ' je veux rejouer alors je reinitialise le jeu
                            ...
                            lbltemp.Text = 0
                        Else
                            pnlJeu.Enabled = True 'je ne veux plus jouer alors je retourne à l'accueil
                            Initialise(Taille, NbMine)
                            For Each element In pnlJeu.Controls
                                element.enabled = True
                                element.text = ""
    ...
    Private Sub Reussi()
        'cette procédure permet de vérifier si j'ai gagné le jeu
